...
const fetchFlightDetails = async () => {
    return {
        departure: {
            airline: "VietJet",
            flightCode: "VJ162",
            departureTime: "2024-01-21T22:05:00",
            basePrice: 1088000,
        },
        return: {
...
const Checkout = () => {
    ...
    const calculateMultiplier = (departureDate, rules) => {
        ...
        const daysBeforeDeparture = Math.ceil((departure - today) / (1000 * 60 * 60 * 24));
        let multiplier = 1.0;

        for (const rule of rules.sort((a, b) => a.daysBeforeDeparture - b.daysBeforeDeparture)) {
    ...
    const calculateTotalPrice = (flights, rules) => {
        const departureMultiplier = calculateMultiplier(flights.departure.departureTime, rules);
        const returnMultiplier = calculateMultiplier(flights.return.departureTime, rules);
        ...
        return Math.round(departurePrice + returnPrice - 20000); // Giảm giá
    };
            ...
            <div className="row">
                ...
                <div className="col">
                    <section className="flight-info">
                        ...
                        <div>
                            ...
                            <p>Thời gian: {new Date(flightDetails.departure.departureTime).toLocaleString()}</p>
                            <p>Giá gốc: {flightDetails.departure.basePrice.toLocaleString()} VND</p>
                        </div>
                        <div>
                            <h5>Chiều về: {flightDetails.return.airline}</h5>
                            <p>Mã chuyến bay: {flightDetails.return.flightCode}</p>
                            <p>Thời gian: {new Date(flightDetails.return.departureTime).toLocaleString()}</p>
                            <p>Giá gốc: {flightDetails.return.basePrice.toLocaleString()} VND</p>
                        </div>
                    </section>

                    <section className="passenger-info">
                        <h4>Thông tin hành khách</h4>
                        <input name="name" placeholder="Họ và tên" onChange={handleInputChange} />
                        <input name="phone" placeholder="Số điện thoại" onChange={handleInputChange} />
                ...
                <div className="col">
                    ...
                    <section className="pricing-summary">
                        <h4>Tóm tắt giá</h4>
                        <p>Chiều đi: {flightDetails.departure.basePrice.toLocaleString()} VND</p>
                        <p>Chiều về: {flightDetails.return.basePrice.toLocaleString()} VND</p>
